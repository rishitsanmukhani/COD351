...
static int caching_list_directory(const char *path, dir_entry **list)
{
  ...
  if (!cw)
  {
    if (!hos_list_directory(path, list))
      return  0;
    cw = new_cache(path);
  }
...
static int cfs_fgetattr(const char *path, struct stat *stbuf, struct fuse_file_info *info)
{
  ...
  if (of)
  {
    stbuf->st_size = hos_file_size(of->fd);
    stbuf->st_mode = S_IFREG | 0666;
    stbuf->st_nlink = 1;
    return 0;
  }
...
static int cfs_open(const char *path, struct fuse_file_info *info)
{
  ...
  {
    if (!hos_object_write_fp(path, temp_file))
    {
      fclose(temp_file);
      return -ENOENT;
    }
...
static int cfs_flush(const char *path, struct fuse_file_info *info)
{
  ...
  if (of)
  {
    update_dir_cache(path, hos_file_size(of->fd), 0);
    if (of->flags & O_RDWR || of->flags & O_WRONLY)
    {
      FILE *fp = fdopen(dup(of->fd), "r");
      rewind(fp);
      if (!hos_object_read_fp(path, fp))
      {
        fclose(fp);
        return -ENOENT;
      }
...
int main(int argc, char **argv)
{
  ...
  if (!*options.username)
  {
    ...
    fprintf(stderr, "  server=[Hadoop object store API server url]\n");
  ...
  hos_set_credentials(options.username, options.server);
